...
# The adapter therefore uses Google's minimum accepted value, while the route
# independently enforces a five-second user-facing deadline. If Gemini has not
# answered after the hedge delay, Groq starts in parallel and the first
# successful provider wins.
...
def generate_extension_content(
    *,
    ...
    """Return the first usable extension response within one shared deadline.

    A slow primary is hedged with the text-only fallback. The executor is not
    allowed to wait for the losing request after a response is ready; each
    adapter has its own network timeout and will finish shortly on its own.
    """
